...
async function syncProjectFromDelivery(delivery, action, actor) {
  ...
  if (action === "approve") {
    if (project.isOngoing) {
      ...
      // Normal project: customer-approved final delivery marks it "delivered".
      project.status = "delivered";
      project.progress = 100;
      project.completedAt = new Date();
      project.history.push({ action: "delivery.approved", message: "Customer approved final delivery — project delivered", userId: actor || null, at: new Date() });
    }
